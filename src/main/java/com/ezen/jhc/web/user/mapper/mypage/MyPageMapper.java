...
public interface MyPageMapper {
	...
	@Select(" select m.mem_num, m.mem_name, ord.ord_date, od.ord_num, p.p_name, pi.origin_img_path, pd.p_num, pc.pc_name, pc.pc_num, ps.ps_num, od.od_num "
			+ "from members m, orders ord, order_details od, prod_details pd, prods p, prod_images pi, prod_colors pc, prod_sizes ps, reviews r "
			+ "where m.mem_num = #{mem_num} and ord_status = 4 and ord.mem_num = m.mem_num and ord.ord_num = od.ord_num and p.p_num = r.p_num and od.od_num != r.od_num "
	...
	// 주문 상품 정보 쿼리문(리뷰 작성용)
	@Select("select p.p_name, pc.pc_name, pi.origin_img_path AS origin_img_path, ps.ps_name, p.p_num "
	...
	List <OrderDetailDTO> mpGetAllReview(@Param("p_num")Integer p_num, @Param("pc_num")Integer pc_num, @Param("ps_num")Integer ps_num);
	
	
	// 작성한 리뷰 추가 쿼리문
	@Insert("INSERT INTO reviews VALUES(review_num_seq.NEXTVAL, "
			+ "#{p_num},#{mem_num},#{review_content},#{review_image_path},#{review_star},sysdate, (SELECT od_num FROM reviews WHERE od_num = #{od_num}))")
